...
public class WeatherProvider extends ContentProvider {
    ...
    private Cursor getLocation(String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        return mOpenHelper.getReadableDatabase().query(LocationEntry.TABLE_NAME,
                projection,
                selection,
                selectionArgs,
                null,
    ...
    private Cursor getWeather(String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        return mOpenHelper.getReadableDatabase().query(WeatherEntry.TABLE_NAME,
                projection,
                selection,
                selectionArgs,
                null,
    ...
    public int update(
        ...
        switch (match) {
            case WEATHER: {
                normalizeDate(values);
                long _id = db.update(WeatherEntry.TABLE_NAME, values, selection, selectionArgs);
                if ( _id <= 0 )
                    throw new android.database.SQLException("Failed to update row into " + uri);
                break;
            }
            case LOCATION: {
                long _id = db.update(LocationEntry.TABLE_NAME, values, selection, selectionArgs);
                if ( _id <= 0 )
                    throw new android.database.SQLException("Failed to update row into " + uri);
                break;
            }
